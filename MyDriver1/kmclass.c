...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// 定义键盘处理程序
NTSTATUS keyHandler(PVOID data) {
	// 输入数据起始和结束指针
	PKEYBOARD_INPUT_DATA KbdInputDataStart, KbdInputDataEnd;
	ULONG InputDataConsumed = 1;  // 表示输入数据被消费的标志
	KIRQL irql;

	// 分配内存以存储键盘输入数据
	PKEYBOARD_INPUT_DATA mid = ExAllocatePool(NonPagedPool, sizeof(KEYBOARD_INPUT_DATA) * 2);
	RtlZeroMemory(mid, sizeof(KEYBOARD_INPUT_DATA) * 2); // 清零分配的内存

	// 复制输入数据到分配的内存中
	memcpy(mid, data, sizeof(KEYBOARD_INPUT_DATA));

	KbdInputDataStart = mid;  // 设置键盘输入数据起始指针
	KbdInputDataEnd = KbdInputDataStart + 1;  // 设置键盘输入数据结束指针

	// 如果存在键盘类服务回调函数，则调用它
	if (g_KoMCallBack.KeyboardClassServiceCallback) {
		KeEnterGuardedRegion();  // 进入保护区域
		KeRaiseIrql(DISPATCH_LEVEL, &irql);  // 提升IRQL到DISPATCH_LEVEL  提升等级后会立马处理以前的DPC队列
		...
		KeLowerIrql(irql);  // 恢复原来的IRQL
		KeLeaveGuardedRegion();  // 离开保护区域
	}
...
// 定义鼠标处理程序
NTSTATUS MouseHandler(PVOID data) {
	// 输入数据起始和结束指针
	PMOUSE_INPUT_DATA MouseInputDataStart, MouseInputDataEnd;
	ULONG InputDataConsumed = 1;  // 表示输入数据被消费的标志
	KIRQL irql;
	...
	// 分配内存以存储鼠标输入数据
	PMOUSE_INPUT_DATA mid = ExAllocatePool(NonPagedPool, sizeof(MOUSE_INPUT_DATA) * 2);
	RtlZeroMemory(mid, sizeof(MOUSE_INPUT_DATA) * 2); // 清零分配的内存

	// 复制输入数据到分配的内存中
	memcpy(mid, data, sizeof(MOUSE_INPUT_DATA));

	MouseInputDataStart = mid;  // 设置鼠标输入数据起始指针
	MouseInputDataEnd = MouseInputDataStart + 1;  // 设置鼠标输入数据结束指针

	// 如果存在鼠标类服务回调函数，则调用它
	if (g_KoMCallBack.MouseClassServiceCallback) {
		KeEnterGuardedRegion();  // 进入保护区域
		KeRaiseIrql(DISPATCH_LEVEL, &irql);  // 提升IRQL到DISPATCH_LEVEL

		// 检查设备扩展是否有效
		if (MmIsAddressValid(g_KoMCallBack.MouDeviceObject->DeviceExtension)) {
			// 调用鼠标类服务回调
			g_KoMCallBack.MouseClassServiceCallback(
				g_KoMCallBack.MouDeviceObject,
				MouseInputDataStart,
				MouseInputDataEnd,
				&InputDataConsumed
			);
